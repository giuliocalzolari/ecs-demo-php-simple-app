...
if [ -z ${AWS_DEFAULT_REGION+x} ]; then AWS_DEFAULT_REGION=false; fi
...
echo "Set Image  $repo/$imend:$tag "
# Replace the image tag in the old def
sed -i "s/\(\"image\": \"$repo\/$imend\):.*\"/\1:$tag\"/g" def
...
do
  ...
  echo "Waiting Update of ECS Cluster"
